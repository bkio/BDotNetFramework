...
namespace BServiceUtilities
{
    ...
    /// 
    /// <para>TRACING_SERVICE_PROVIDER</para>
    /// <para>TRACING_SERVICE_PROVIDER can be ZIPKIN or GC</para>
    /// 
    /// <para>If TRACING_SERVICE_PROVIDER is ZIPKIN;</para>
    ...
    public partial class BServiceInitializer
    {
        ...
        {
            if (!RequiredEnvironmentVariables.ContainsKey("TRACING_SERVICE_PROVIDER"))
            {
                LoggingService.WriteLogs(BLoggingServiceMessageUtility.Single(EBLoggingServiceLogType.Critical, "TRACING_SERVICE_PROVIDER environment variable is missing."), ProgramID, "Initialization");
                return false;
            }

            var TracingServiceProvider = RequiredEnvironmentVariables["TRACING_SERVICE_PROVIDER"];

            /*
            * Tracing service initialization
            */
            if (TracingServiceProvider == "ZIPKIN")
            {
            ...
            else if (TracingServiceProvider == "GC")
            {
